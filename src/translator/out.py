...
class Array(list):
    ...
    def reduce(self, callback):
        ...
        for x in self:
            a = callback(a, x)
        return a
def edmimqsylqvdtksxjhwvqouhwolytgsv(a,b): return (a+b)
t=Array()
def errr(arr):
    for j in arr:
        if not Array((0),(1),(2),(3),(4),(5)).includes(j):return ((True))
    return ((False))
while ((True)):
    d=float(input())
    if (d==-(1)):break
    t.push(d)
if t.includes((0)):print(("neklasifikovan"))
elif errr(t):print(("error"))
else:
    print((str((round(((t.reduce(edmimqsylqvdtksxjhwvqouhwolytgsv)/t.length)*(1e2)))/(1e2)))+("0")))