...
class Array(list):
    ...
    def reduce(self, callback):
        ...
        return a
import math
def eilmwsyflyernxhwwyxieivtpfshpmpi(v): return Animal(v)
def msmtmtdhrknpfnytentynxinzlyzpjyr(v): return Animal(v)
def mijfkwntiwzkxxvjkmetgztgoglhgizo(v): return v.id
def uhjdhfptsdvvsxmidxqkuqxogupfsvdk(v): return (v.id==String("tiger"))
def iygncntjnqmilugffmymqtmdpwppglls(v): return v.id
def qooldzihqykeyzddspwsmgexzwpwhnoq(v): return v.id
def hsmlyjrecpgqqphksrrkitzkjgvzxsyg(v,i): return ((i%Number(2))==Number(0))
def rledzinutvphemlwivoffgmzxmtqmkiz(v): return v.id
def zrcqjdsffzklejsgmxjnmudozxjodnjl(v): return v.id.toLowerCase().startsWith(String("l"))
def qqqekivljqndptekwwrgfjdhsuidetuw(v): return v.id
def rkhgirdsuncxrjyqgidfzrqzjlfcmdzd(v): return v.id.toLowerCase().startsWith(String("l"))
def kxfzmuvmmymnlgsspvkelmdnrtxuwhys(v): return v.id
def qtoueiehlezuhsqeheuxpffwqhfchurf(v): return v.id
def mkimcjkfvyvgtksfpvzsfohnxwmwqsqw(v): return v.id.toLowerCase().startsWith(l)
def ghqshelfsnwcoeceshxwulngfoouhohi(v): return v.id
def pmfpqunqkocqeqjyciwnnrzvehlnkogh(v): return v.id.toLowerCase().startsWith(l)
def qtsyuqwsnmujjdsztqunnhvpnstidolk(v): return v.id
def eodnwvwgkxekmpwuediijovixckqmdne(v): return v.id
def dhurqgeynsxhssecmwjvygorlwgdriiu(v): return v.id
class Animal:
    def __init__(self, id):
        self.id=id
    def toString(self, ):
        return (String("Animal: ")+self.id)
animals=Array(String("lovebird"),String("bison"),String("prairie dog"),String("chimpanzee"),String("pig"),String("bighorn"),String("capybara"),String("fawn"),String("armadillo"),String("wolf"),String("dromedary"),String("aoudad"),String("monkey"),String("guanaco"),String("guanaco"),String("ape"),String("doe"),String("bumble bee"),String("iguana"),String("highland cow"),String("snowy owl")).map(eilmwsyflyernxhwwyxieivtpfshpmpi)
animals2=Array(String("tiger"),String("elephant"),String("tiger"),String("lion"),String("locust"),String("lion"),String("eagle"),String("elephant"),String("echidna"),String("eagle"),String("robin"),String("chimpanzee"),String("sloth"),String("crow"),String("locust"),String("dolphin"),String("gorilla"),String("wolf"),String("lovebird"),String("owl"),String("ostrich"),String("badger"),String("anteater"),String("kookaburra")).map(msmtmtdhrknpfnytentynxinzlyzpjyr)
print((String("2)\n     ")+animals.map(mijfkwntiwzkxxvjkmetgztgoglhgizo)))
print((String("3)\n     ")+animals.filter(uhjdhfptsdvvsxmidxqkuqxogupfsvdk).length))
print((String("4)\n     ")+animals[Number(2)].id))
print((String("5)\n     ")+animals[Number((animals.length-Number(2)))].id))
print((String("6)\n     ")+animals.slice(Number(0),Number(10)).map(iygncntjnqmilugffmymqtmdpwppglls)))
print((String("7)\n     ")+animals.slice(Number((animals.length-Number(5))),Number(animals.length)).map(qooldzihqykeyzddspwsmgexzwpwhnoq)))
print((String("8)\n     ")+animals.filter(hsmlyjrecpgqqphksrrkitzkjgvzxsyg).map(rledzinutvphemlwivoffgmzxmtqmkiz)))
print((String("9)\n     ")+animals.filter(zrcqjdsffzklejsgmxjnmudozxjodnjl).map(qqqekivljqndptekwwrgfjdhsuidetuw)))
print((String("10)\n     ")+animals.filter(rkhgirdsuncxrjyqgidfzrqzjlfcmdzd).map(kxfzmuvmmymnlgsspvkelmdnrtxuwhys)))
print(String("11)"))
max=Number(0)
maxL=Number(0)
...
print((String("12)\n     ")+animals.slice(Number((maxLastIndex-max)),Number(maxLastIndex)).map(qtoueiehlezuhsqeheuxpffwqhfchurf)))
...
def printStartingWith(l):
    print(((String("     ")+l)+String(":")),animals.filter(mkimcjkfvyvgtksfpvzsfohnxwmwqsqw).map(ghqshelfsnwcoeceshxwulngfoouhohi))
for l in String("dikobraz").split(String("")):
    printStartingWith(l)
print(String("15)"))
def printStartingWith(l):
    print(((String("     ")+l)+String(":")),animals.filter(pmfpqunqkocqeqjyciwnnrzvehlnkogh).map(qtsyuqwsnmujjdsztqunnhvpnstidolk))
for l in String("abcdefghijklmnopqrstuvwxyz").split(String("")):
    printStartingWith(l)
print((String("16)\n     ")+Array(*animals,*animals2).map(eodnwvwgkxekmpwuediijovixckqmdne)))
print((String("17)\n     ")+Array(*animals.slice(Number(0),Number((animals.length/Number(3)))),*animals2.slice(Number(((Number(2)*animals.length)/Number(3))),animals.length)).map(dhurqgeynsxhssecmwjvygorlwgdriiu)))