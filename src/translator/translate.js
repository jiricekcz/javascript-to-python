...
var usedIdentifiers = [];
module.exports = async function translate(javascriptString, cdnLink) {
    var program = acorn.parse(javascriptString, { ecmaVersion: 2020 });
    if (cdnLink) {
        var link = await discord(javascriptString, "source.js");
        globalImports.push(`# Discord CDN link to original source: ${link}`);
    }
...
function requireStatement(id) {
    ...
    if (!dictionary.supportedRequireModules.includes(id)) {
        throw new Error("Module " + id + " has not been trasnlated to python.")
    }
    addGlobalImport(fs.readFileSync(path.join(__dirname, "./python/" + id + ".py")));
    return id;
}